...
static const MemMapEntry virt_memmap[] = {
    ...
    // 3 PLIC context per hart: M, S, U
    [VIRT_PLIC] =        {  0xc000000, VIRT_PLIC_SIZE(VIRT_CPUS_MAX * 3) },
    [VIRT_APLIC_M] =     {  0xc000000, APLIC_SIZE(VIRT_CPUS_MAX) },
    [VIRT_APLIC_S] =     {  0xd000000, APLIC_SIZE(VIRT_CPUS_MAX) },
    [VIRT_UART0] =       { 0x10000000,         0x100 },
    [VIRT_VIRTIO] =      { 0x10001000,        0x1000 },
    [VIRT_UART1] =       { 0x10009000,         0x100 },
    [VIRT_UART2] =       { 0x1000A000,         0x100 },
    [VIRT_UART3] =       { 0x1000B000,         0x100 },
    [VIRT_UART4] =       { 0x1000C000,         0x100 },
                            ...
                            uint32_t irq_mmio_phandle)
{
    ...
    // add uart 1-4
    for (int uart_i = VIRT_UART1; uart_i <= VIRT_UART4; ++uart_i) {
        name = g_strdup_printf("/soc/serial@%lx", (long)memmap[uart_i].base);
        qemu_fdt_add_subnode(mc->fdt, name);
        qemu_fdt_setprop_string(mc->fdt, name, "compatible", "ns16550a");
        qemu_fdt_setprop_cells(mc->fdt, name, "reg",
            0x0, memmap[uart_i].base,
            0x0, memmap[uart_i].size);
        // 100M clock
        qemu_fdt_setprop_cell(mc->fdt, name, "clock-frequency", 3686400);
        qemu_fdt_setprop_cell(mc->fdt, name, "interrupt-parent", irq_mmio_phandle);
        if (s->aia_type == VIRT_AIA_TYPE_NONE) {
            qemu_fdt_setprop_cell(mc->fdt, name, "interrupts", UART1_IRQ + uart_i - VIRT_UART1);
        } else {
            qemu_fdt_setprop_cells(mc->fdt, name, "interrupts", UART1_IRQ + uart_i - VIRT_UART1, 0x4);
        }
    }
...
static void virt_machine_init(MachineState *machine)
{
    ...
    for (int uart_i = 1; uart_i <= 4; ++uart_i) {
        serial_mm_init(system_memory, memmap[VIRT_UART0 + uart_i].base,
            0, qdev_get_gpio_in(DEVICE(mmio_irqchip), UART1_IRQ + uart_i - 1), 399193,
            serial_hd(uart_i), DEVICE_LITTLE_ENDIAN);
    }
